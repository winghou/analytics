...
const {sevenStages} = require('./util/Stages');

/** 
 * 七天错误数据走势
 * @module sevenErrorLine
 * @param {参数类型} 参数名 参数说明
 * @return {返回值类型} 返回值说明
 */
let sevenErrorLine = function(req, res, mydb) {
    let reqQuery = req.query,
        ...
        stages = sevenStages( AfterDateTimestamps[0],Timestamps[1],url);
     ...
     Promise.all([
            ...
            (day<7)?save_minitor.model("errorList" + LastMonth[0] + '_' + LastMonth[1], mydb.errorSchema).aggregate(stages):[]
        ])
        .then(function (data) {
